...
export let Auth = createAsyncThunk(
  "role",
...
let authSlice = createSlice({
  ...
  extraReducers: {
    ...
    [Auth.fulfilled]: (state, action) => {
      ...
      state.Loading = false;
      state.Error = false;
      state.Token = window.localStorage.setItem(
        "AuthTokenUser",
        action.payload.verifyCodeSuccess
      );
      state.UserData = window.localStorage.setItem(
        "AuthDataUser",
        JSON.stringify(action.payload.verifyUserData)
      );
