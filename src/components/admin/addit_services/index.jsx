...
const AdditServiesComponent = ({ order }) => {
  ...
  const HandleSubmitMiniBar = async e => {
    ...
    await axios.put(`${API_URL}/products/${products}`, bodys).then(res => res);
    let bodyPrices = {
      paid: Number(pricesMinibar.current.value) + Number(OrderGet.filter(e => e.id === orders)[0].paid),
      total_payable:
        Number(pricesMinibar.current.value) + Number(OrderGet.filter(e => e.id === orders)[0].total_payable),
    };
    const bodyPrices2 = {
      debt: Number(pricesMinibar.current.value) + Number(OrderGet.filter(e => e.id === orders)[0].debt),
      total_payable:
        Number(pricesMinibar.current.value) + Number(OrderGet.filter(e => e.id === orders)[0].total_payable),
    };
    if (statusPayment === 'Оплачено') {
      await axios.put(`${API_URL}/orders/${orders}`, bodyPrices).then(res => res);
    } else {
      await axios.put(`${API_URL}/orders/${orders}`, bodyPrices2).then(res => res);
    }
  ...
  const HandleSubmitLaundry = async e => {
    ...
    let bodyPrices = {
      paid: Number(prices) + Number(OrderGet.filter(e => e.id === orders)[0].paid),
      total_payable: Number(prices) + Number(OrderGet.filter(e => e.id === orders)[0].total_payable),
    };
    const bodyPrices2 = {
      debt: Number(prices) + Number(OrderGet.filter(e => e.id === orders)[0].debt),
      total_payable: Number(prices) + Number(OrderGet.filter(e => e.id === orders)[0].total_payable),
    };
    if (statusPayment === 'Оплачено') {
      await axios.put(`${API_URL}/orders/${orders}`, bodyPrices).then(res => res);
      ...
      <div className={styles.form_wrapper}>
        <div className={styles.wrapper}>
                ...
                <div className={styles.right_box}>
                  ...
                  <div className={styles.inputdiv}>
                    ...
                    <input
                      ...
                      ref={pricesMinibar}
                      value={
                        ProductGet.filter(elem => elem.id === products)[0]?.price === undefined
                          ? 0
                          : ProductGet.filter(elem => elem.id === products)[0]?.price * counts
                      }
                      placeholder="0"
                    />
                  </div>
          ...
          <div className={styles.item_box}>
            ...
            <form
              className={styles.form}
              onSubmit={HandleSubmitPenalties}
            >
              <div className={styles.item_boxs}>
                <div className={styles.right_box}>
                  <SelectCommon
          ...
          <div className={styles.item_box}>
            ...
            <form
              className={styles.form}
              onSubmit={HandleSubmitOtherExpenses}
            >
              <div className={styles.item_boxs}>
                <div className={styles.right_box}>
                  <SelectCommon
